...
export default function Product() {
    ...
    const handleProdStatus = (prodId, newStatus) => {
        setLoading(true)
        axios.put(`/product/${prodId}/0`, { status: newStatus })
            .then(() => {
                fetchProdData();
                setLoading(false);
            })
    ...
    const handleProdSlider = (prodId, newSlider) => {
        setLoading(true);
        axios.put(`/product/${prodId}/1`, { slider: newSlider })
            .then(() => {
                fetchProdData();
                setLoading(false);
            })
    ...
    const handleProdPopular = (prodId, newPopular) => {
        setLoading(true);
        axios.put(`/product/${prodId}/2`, { popular: newPopular })
            .then(() => {
                fetchProdData();
                setLoading(false);
            })
                            ...
                            return (
                                <tr key={product.prod_id} className='brand-row'>
                                    ...
                                    <td onClick={() => { handleViewData(product.prod_id) }}>
                                        {product.image && (
                                            <img src={require(`../../Assets/upload/${product.image}`)} alt="" width="88px" />
                                        )}
                                    </td>
                                    <td>
                                        {isActive === 1 ? (
                                    ...
                                    <td>
                                        {isPopular === 1 ? (
                                            <div className="form-check">
                                                <input className="form-check-input" type="checkbox" id="active" onChange={() => { handleProdPopular(product.prod_id, 0) }} checked={true} />
                                            </div>
                                        ) : (
                                            <div className="form-check">
                                                <input className="form-check-input" type="checkbox" id="inactive" onChange={() => { handleProdPopular(product.prod_id, 1) }} checked={false} />
                                            </div>
                                        )}
                                    </td>
                                </tr>
