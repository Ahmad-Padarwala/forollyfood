...
export default function BotSlider() {
    ...
    const saveSliderData = (e) => {
        ...
        axios.post('/bot-slider', formData)
            .then(() => {
                setTimeout(() => {
                    fetchBottomSlider();
                }, 1000);
    ...
    const handleDelete = (deleteId) => {
        axios.delete(`/bot-slider/${deleteId}`)
            .then(() => {
                fetchBottomSlider();
                setOpen(false);
            })
    ...
    const handleSaveEditData = (id) => {
        ...
        setLoading(true);
        axios.put(`/bot-slider/${id}`, formData)
            .then(() => {
                setTimeout(() => {
                    fetchBottomSlider();
                }, 1300);
                        ...
                        botSlid.map((slider) => {
                            return (
                                <tr key={slider.id} className='brand-row'>
                                    ...
                                    <td>
                                        {slider.image && (
                                            <img src={require(`../../Assets/upload/${slider.image}`)} alt="" width="65px" />
                                        )}
                                    </td>
                                </tr>
                            )
